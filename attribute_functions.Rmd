...
## The perturbation, multiplication and powering
...
It returns one matrix for each observation: **is all this OK?**
...
```{r}
Btest = xy_reg(Y=m1,X=list(m2))$B_ilr
...
```{r}
xy_impact_coef = function(Y,X,Z=list(matrix()),V=list(matrix()), constant=TRUE, B, Bspace="ilr"){ #Bspace could be 'ilr' or 'simplex' and specifies which kind of matrix of coefficients is given.
...
Btestilr     = xy_reg(Y=m1, X=list(m2), Z=list(m5))$B_ilr
```
...
xy_impact_coef(Y=m1,X=list(m2), Z=list(m5), B=Btestilr, Bspace="ilr")
```
