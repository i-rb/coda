...
```{r}
impacts(ilr_x_reg(m5,list(m1,m4),list(m3)))
```

```{r}
impacts(reg(dataset = BDDSegX, S_A + S_B ~ cbind(PIB_Courant_t,  HT_Gazole, FBCF_Men_Courant), "ILR"))
```
...
```{r}
check_formula(BDDSegX, V_A + V_C ~ HT_Gazole + HT_SP95)$model_type
```

```{r}
check_formula(BDDSegX, cbind(V_A, V_C) ~ HT_Gazole + HT_SP95 + PIB_Courant_t)$model_type
```
...
```{r}
impacts(reg(BDDSegX, HT_Gazole ~ cbind(S_A,S_B) + cbind(S_C,S_D) + PIB_Courant_t))
```
...
# pres = TRUE to print with a better presentation (name of variables and sd if it is the case) of B and B*.
  ...
  list_return = list(Y_coord = ty, X_coord = tx, B_coord = Bstar, B_simplex = Bs, Bcoord_cov=bcov, residuals_coord = resid, fitted_v_coord = fittedvalues)
...
```{r}
ilr_y_impacts(ilr_y_reg(m1,list(m3)))[[1]]
```


```{r}
ilr_y_impacts(ilr_y_reg(m1,list(m3,m5)))
```
...
```{r}
imp_summary = function(impact_values){
  dim1 = dim(impact_values[[1]])[1]
  dim2 = dim(impact_values[[1]])[2]
  maxmat = matrix(0,dim1,dim2)
  q3mat = matrix(0,dim1,dim2)
  medmat = matrix(0,dim1,dim2)
  q1mat = matrix(0,dim1,dim2)
  minmat = matrix(0,dim1,dim2)
  for (i in (1:dim1)){
    for (j in (1:dim2)){
      values = numeric(length(impact_values))
      for (k in (1:length(impact_values))){
        values[k] = impact_values[[k]][i,j]
      }
      maxmat[i,j] = max(values)
      q3mat[i,j] = quantile(values,0.75)
      medmat[i,j] = quantile(values,0.5)
      q1mat[i,j] = quantile(values,0.25)
      minmat[i,j] = min(values)
    }
  }
...
```{r}
imp_summary(impacts(ilr_yx_reg(m1,list(m2), list(m5))))
```
