...
ilr_yx_reg = function(Y,X,Z=list(matrix()),V=list(matrix()),constant=TRUE, pres=FALSE){ # Y dependent, X indep compositional, Z indep. not compositional, c if constant is included, V list of contrast matrices. order: (Y, X1, X2, ...) 
...
```{r}
alr_yx_reg = function(Y,X,Z=list(matrix()),R=list(NULL),constant=TRUE, pres=FALSE){ # Y dependent, X indep compositional, Z indep. not compositional, c if constant is included, V list of reference variables order: (Y, X1, X2, ...) 
# pres = TRUE to print with a better presentation (name of variables and sd if it is the case) of B and B*.
...
```{r}
ilr_x_reg = function(Y,X,Z=list(matrix()),V=list(matrix()),constant=TRUE, pres=FALSE){ # Y dependent, X indep compositional, Z indep. not compositional, c if constant is included, V list of contrast matrices. order: (Y, X1, X2, ...) 
...
```{r}
alr_x_reg = function(Y,X,Z=list(matrix()),R=list(NULL),constant=TRUE, pres=FALSE){ # Y dependent, X indep compositional, Z indep. not compositional, c if constant is included, V list of contrast matrices. order: (Y, X1, X2, ...) 
# pres = TRUE to print with a better presentation (name of variables and sd if it is the case) of B and B*.
...
```{r}
reg = function(dataset, formula, transformation = "ILR", pres=FALSE, V=list(matrix()), R=list(NULL)){
  ...
  if (transformation == "ILR"){
    if (model_type == "yx_reg"){
      if (pres){
      cat("YX-compositional model. ILR transformation. \n", "========================================== \n")
      }
      return(ilr_yx_reg(Y=Y,X=X,Z=Z,V=V,constant = constant, pres = pres))
    }
    if (model_type == "x_reg"){
      if (pres){
      cat("X-compositional model. ILR transformation. \n", "========================================== \n")
      }
      return(ilr_x_reg(Y=Y,X=X,Z=Z,V=V,constant=constant,pres=pres))
    }
    if (model_type == "y_reg"){
      if (pres){
      cat("Y-compositional model. ILR transformation. \n", "========================================== \n")
      }
  ...
  if (transformation == "ALR"){
    if (model_type == "yx_reg"){
      if (pres){
      cat("YX-compositional model. ALR transformation. \n", "========================================== \n")
      }
      return(alr_yx_reg(Y=Y,X=X,Z=Z,R=R,constant=constant,pres=pres))
    }
    if (model_type == "x_reg"){
      if (pres){
      cat("X-compositional model. ALR transformation. \n", "========================================== \n")
      }
      return(alr_x_reg(Y=Y,X=X,Z=Z,R=R,constant=constant,pres=pres))
    }
    if (model_type == "y_reg"){
      if (pres){
      cat("Y-compositional model. ALR transformation. \n", "========================================== \n")
      }
...
```{r}
ilr_y_reg = function(Y,X,V=list(matrix()),constant=TRUE, pres=FALSE){ # Y dependent, X indep. not compositional, c if constant is included, V list of contrast matrices. order: (Y, X1, X2, ...) 
# pres = TRUE to print with a better presentation (name of variables and sd if it is the case) of B and B*.
...
```{r}
alr_y_reg = function(Y,X,R=list(NULL),constant=TRUE, pres=FALSE){ # Y dependent, X indep. not compositional, c if constant is included, V list of contrast matrices. order: (Y, X1, X2, ...) 
# pres = TRUE to print with a better presentation (name of variables and sd if it is the case) of B and B*.
...
if (pres=="TRUE"){
    ...
    nc = dim(dfalr)[1]
    for (i in dimilr:1){
      if (i==dimilr){dfalr = cbind(dfalr,matrix(nrow=nc))}
      else{
        dfalr = cbind(dfalr[,1:i],matrix(nrow=nc),dfalr[,(i+1):(dim(dfalr)[2])])
      }
    }
...
p_values_coef = function(dataset, formula, transformation = "ILR", V=list(matrix()), R=list(NULL), B=1000, seed=31000){
  ...
  for (b in 1:B){
    ...
    coefs_b = reg(dataset = sample_b, formula = formula, transformation = transformation, V=V, R=R, pres=F)$B_simplex
    results[[b]] = coefs_b
  }
  pvals = results[[1]]
  ...
  cat("P-values for coefficients (H0: coef = 1/#parts):\n\n")
  return(pvals)
}
```
...
```{r}
reg2 = function(dataset, formula, transformation = "ILR", V=list(matrix()), R=list(NULL), B=1000, seed=31000){
  reg_res = reg(dataset = dataset, formula = formula, transformation = transformation, V=V, R=R, pres=F)
  input = check_formula(dataset, formula)
  ...
  pvalues = p_values_coef(dataset,formula,transformation,V,R,B,seed)
  ...
  for (i in dim(sigmat)[2]:1){
    ...
    #nnames_i  = paste("(sig. ",orignames[[i]],")",sep="")
    nnames_i = "   "
    if (i==dim(sigmat)[2]){
...
p_values_se = function(dataset, formula, observation, transformation = "ILR", V=list(matrix()), R=list(NULL), B=1000, seed=31000){
  ...
  for (b in 1:B){
    ...
    sample_b <- dataset[ids,]
    rownames(sample_b) = NULL
    impacts_i = impacts(reg(dataset = sample_b, formula = formula, transformation = transformation, V=V, R=R, pres=F))[[1]]
    results[[b]] = impacts_i
  }
  rimpacts = results[[1]]
  ...
  cat("P-values for (semi-)elasticities (H0: s.e. = 0): \n\n")
  return(rimpacts)
}
```
...
This should be pooled into one pvalues function that checks the model,
also ratio_share/imp_summary should raise an error if it is not a Y-compo model (or YX maybe?)
...
```{r}
pvalues_x = function(dataset, formula, transformation = "ILR", V=list(matrix()), R=list(NULL), B=1000, seed=31000){
  results = list()
  set.seed(seed)
  for (b in 1:B){
    ids <- sample(x=nrow(dataset), size=dim(dataset)[1], replace=T)
