...
This notebook is only to create the package. Be careful! It ends with 'package-skeleton'.
...
```{r}
alr = function(x,R=dim(x)[2]){ # x: N x D matrix, R : Reference (last term as default)
  if (is.null(attr(x, "space"))){attr(x, "space") <- "simplex"}
...
```{r}
impacts = function(results, p_values=FALSE){
...
```{r}
reg = function(dataset, formula, transformation = "ILR", V=list(matrix()), R=list(NULL), pres=FALSE, simplex_pvalues=FALSE, B=1000, seed=31000){
  ...
  for (i in dim(ilr_pvals)[2]:1){
    ...
    if (simplex_pvalues=="exact"){
      ...
      if (model_type=="x_reg"){
        ...
        counter = 1
        if (constant){
          temp_vec = numeric(dim(dfilr)[2]/3)
          temp_vec2 = numeric(dim(dfilr)[2]/3)
          for (k in 1:(dim(dfilr)[2])){
            if (k%%3==0){
              temp_vec[k/3]=dfilr[1,k]
            }
          }
          for (k in 1:(dim(dfilr)[2])){
            if (k%%3==2){
              temp_vec2[round(k/3)]=dfilr[1,k]
            }
          }
          ccc = 2
          counter = 2
        ...
        yparts = dim(Bs)[2]
        for (dimx in reg_res$xdim){
          for (hh in 1:yparts){
            dimbcovy = dim(bcov)[2] / yparts
            bcov2 = bcov[((hh-1)*dimbcovy+1):((hh)*dimbcovy),((hh-1)*dimbcovy+1):((hh)*dimbcovy)]
            bcovred = bcov2[ccc:(ccc+dimx-2),ccc:(ccc+dimx-2)]
            Vred = V_D(dimx)
            Bstarred = Bstar[ccc:(ccc+dimx-2),]
            subcase = 0
            Bstarred_or = Bstarred
            if (is.null(dim(Bstarred))){
              if (dim(Vred)[2]!=1){
                Bstarred = matrix(Bstarred)
                subcase = 1
              }
              if (dim(Vred)[2]==1){
                Bstarred = t(matrix(Bstarred))
                subcase = 2
              }
            }
            jacobian = Vred
            for (i in 1:(dim(jacobian)[1])){
              for (j in 1:(dim(jacobian)[2])){
                if (subcase==1){jacobian[i,j] = (Vred[i,j] + 1)*exp(Vred[i,]%*%Bstarred)}
                jacobian[i,j] = (Vred[i,j] + 1)*exp(Vred[i,]%*%Bstarred[,hh])
                }
            }
            for (i in 1:(dim(jacobian)[1])){
              jacobian[i,] = jacobian[i,]/sum(jacobian[i,])
            }
            denoms = diag(jacobian%*%bcovred%*%t(jacobian))
            var_mat[counter:(counter + dimx - 1), hh] = (Bs[counter:(counter + dimx - 1), hh] - 1/dimx) / sqrt(denoms)
            sds_mat[counter:(counter + dimx - 1), hh] = sqrt(denoms)
          }
      ...
      for (i in 1:(dim(var_mat)[1])){
        for (j in 1:(dim(var_mat)[2])){
          val = var_mat[i,j]
          pval = 1+pnorm(-abs(val))-pnorm(abs(val))
          if (is.na(pval)){pvaluesmat[i,cc]  = " - "}
          else{
            if ((pval>0.05)&(pval<=0.10)){
              pvaluesmat[i,j] = "*  "
            }
      ...
      if (constant){pvaluesmat[1,] = temp_vec}
      if (constant){sds_mat[1,] = temp_vec2}
      
      totcompo = reg_res$xdim
      n_compo = 0 
      for (i in totcompo){
        n_compo = n_compo + i
      }
      if (constant){n_compo = n_compo + 1}
      Bsdim = dim(Bs)[1]
      non_compo = Bsdim - n_compo 
      if (non_compo!=0){
        ilrmatsds = dfilr[(dim(dfilr)[1]-non_compo+1):dim(dfilr)[1],]
        if (is.null(dim(ilrmatsds)[1])){
          ilrmatsds = t(as.matrix(ilrmatsds))
        }
        for (i in 1:(dim(ilrmatsds)[1])){
          for (j in 1:(dim(ilrmatsds)[2])){
            if (j%%3==2){
              sds_mat[n_compo + i,round(j/3)] = ilrmatsds[i,j]
              pvaluesmat[n_compo + i,round(j/3)] = ilrmatsds[i,j+1]
            }
          }
        }
      }
  ...
  if (simplex_pvalues=="exact"){
    for (i in dim(pvaluesmat)[2]:1){
      orignames = colnames(dfsimplex)
      nnames_i = c(paste("(sd_",orignames[[i]],")",sep=""),"   ")
      if (i==dim(pvaluesmat)[2]){
        newnames  = c(orignames[1:i],nnames_i)
        newmat    = cbind(dfsimplex[,1:i],sds_mat[,i],pvaluesmat[,i])
      }
      else{
        newnames  = c(orignames[1:i],nnames_i,orignames[(i+1):length(orignames)])
        newmat    = cbind(dfsimplex[,1:i],sds_mat[,i],pvaluesmat[,i],dfsimplex[,(i+1):(dim(dfsimplex)[2])])
      }
      dfsimplex = newmat
      orignames = newnames
      colnames(dfsimplex) = orignames
...
## pvalues semielasticities by bootrstrap
...
### p values for coefficients via bootstrapping
...
## standard deviation for x compo models

```{r}
sd_se_x = function(reg_results){
...
## Function to present results in a more elegant way

```{r}
tidy = function(matrix, dig=14){
