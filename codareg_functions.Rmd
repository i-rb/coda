...
ilr_x_reg = function(Y,X,Z=list(matrix()),V=list(matrix()),constant=TRUE){
  ...
  reslist = list(Y_coord = ty, X_coord = tx, constant = constant, B_coord = Bstar, B_simplex = Bs , Bcoord_cov=bcov, residuals_coord = resid, fitted_v_coord = fittedvalues, xdim=xdimlist)
...
reg = function(dataset, formula, transformation = "ILR", V=list(matrix()), R=list(NULL), pres=FALSE, simplex_pvalues=FALSE, B=1000, seed=31000){
  ...
  for (i in dim(ilr_pvals)[2]:1){
    ...
    if (simplex_pvalues=="bootstrap"){
      sigmat = dfsimplex
      if (model_type=="y_reg"){pvals = p_values_coef_y(dataset = dataset,formula = formula,transformation = transformation,V=V,R=R,B=B,seed=seed)}
      if (model_type=="yx_reg"){pvals = p_values_coef_y(dataset = dataset,formula = formula,transformation = transformation,V=V,R=R,B=B,seed=seed)}
...
```{r}
p_values_coef_y = function(dataset, formula, transformation = "ILR", V=list(matrix()), R=list(NULL), B=1000, seed=31000){
  results = list()
  model_type = check_formula(dataset,formula)$model_type
  set.seed(seed)
  for (b in 1:B){
    ids <- sample(x=nrow(dataset), size=dim(dataset)[1], replace=T)
    sample_b <- dataset[ids,]
...
## exact semi-elasticities for x compo models

```{r}
se_x = function(reg_results){ 
  ...
  for (k in 1:y_parts){
    ...
    for (j in 1:length(xdim)){
      ...
      var_j = V_D(xdim[[j]])%*%redmat%*%t(V_D(xdim[[j]]))
