...
```{r} 
as.comp <- function(x){
  if(!inherits(x, "comp")) class(x) <- c("comp", class(x))
  C(x)
}
...
Two differences stand out. The first is the function for defining the class. I prefer 'their' form (acomp(x1) or comp(x1) than as.comp(x1)). Second, the object created in our class has two classes, as it retains the original one. Is this better or worse?

I will correct the former.
